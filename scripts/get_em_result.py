...
def parse_args():
    ex = "python get_em_result.py -b ../UNET_EM_DATASET/ -c UNET_EM_DATASET_BNN -s 0"
    parser = argparse.ArgumentParser(ex)
    parser.add_argument('-b', '--base_em_dir', help='path to em experiment folder', required=False, default="/users/hemmat/MyRepos/UNET_Quantized/UNET_EM_DATASET/")
...
def get_input_data(data_set_path="/export/tmp/hemmat/datasets/fake_em/"):
    global is_cuda
    ...
    emdataset_val = EMdataset(image_paths=image_val_path, target_paths=target_val_path)
    val_loader = DataLoader(emdataset_val, batch_size=4, shuffle=True, num_workers=1)
...
def save_image(in_img, gt_img, pred_img, path, config, rand_indx):
    # import ipdb as pdb; pdb.set_trace()
    img_name      = "EM_UNET_"      + config['experiment_name'] + "_" +str(rand_indx) + ".png"
    orig_img_name = "EM_UNET_ORIG_" + config['experiment_name'] + "_" +str(rand_indx) + ".png"
    gt_img_name   = "EM_UNET_GT_"   + config['experiment_name'] + "_" +str(rand_indx) + ".png"
    pred_img_name = "EM_UNET_PRED_" + config['experiment_name'] + "_" +str(rand_indx) + ".png"
    gt_img   *= 255
    pred_img *= 255
    in_img   *= 255
