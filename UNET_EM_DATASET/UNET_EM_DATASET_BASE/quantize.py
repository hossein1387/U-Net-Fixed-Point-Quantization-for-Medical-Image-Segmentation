...
def to_fixed_point(v, i, f):
    ...
    max_float = Variable(torch.zeros(v.size()).type(v.data.type()).cuda() + torch.pow(2*torch.ones(pows.size()).cuda(), pows.float().cuda()).sum()) if f!=0 else Variable(torch.zeros(v.size()).type(v.data.type()).cuda())
    max_int   = Variable(torch.zeros(v.size()).type(v.data.type()).cuda() + 2**(i-1)) if i!=0 else Variable(torch.zeros(v.size()).type(v.data.type()).cuda())
